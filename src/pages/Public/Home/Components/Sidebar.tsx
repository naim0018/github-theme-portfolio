...
import { Mail, Users, MapPin, Briefcase, Link as LinkIcon, Twitter, Star, GitPullRequest, Heart, Bug } from 'lucide-react';
  ...
  return (
    <aside className="flex flex-col gap-6 lg:gap-8 min-w-[280px]">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="pt-2">
          <h1 className="text-2xl font-bold mb-0.5 tracking-tight">Alex Developer</h1>
          <p className="text-xl text-fg-muted font-light mb-4 text-opacity-80">alexdev</p>
          <p className="text-[15px] leading-relaxed mb-6 text-fg-default/90">
            Senior Full Stack Engineer | Open Source Contributor | Tech Speaker 🚀 Building scalable systems & mentoring devs
          </p>
          <button 
            onClick={onHireClick} 
      ...
      <div className="flex items-center gap-4 text-sm text-fg-muted">
        <div className="flex items-center gap-1 cursor-pointer hover:text-accent-fg transition-colors">
          ...
          <span><strong className="text-fg-default">2.4k</strong> followers</span>
        </div>
        <span className="text-border-default">•</span>
        <div className="flex items-center gap-1 cursor-pointer hover:text-accent-fg transition-colors">
          <span><strong className="text-fg-default">180</strong> following</span>
        </div>
      </div>
      
      {/* Meta Information */}
      <div className="flex flex-col gap-3.5 text-[14px] text-fg-default/90">
        <MetaItem icon={<MapPin className="w-4 h-4" />} text="San Francisco, CA" />
        <MetaItem icon={<Briefcase className="w-4 h-4" />} text="Available for opportunities" />
        <MetaItem 
          icon={<LinkIcon className="w-4 h-4" />} 
          text={<a href="#" className="font-semibold text-fg-default hover:text-accent-fg hover:underline transition-colors">alexdev.dev</a>} 
        />
        <MetaItem 
          icon={<Twitter className="w-4 h-4" />} 
          text={<a href="#" className="font-semibold text-fg-default hover:text-accent-fg hover:underline transition-colors">@alexdev</a>} 
        />
      </div>
      ...
      <div>
        <h3 className="text-sm font-semibold mb-5 text-fg-default uppercase tracking-wider opacity-60">Achievements</h3>
      ...
      <div>
        <h3 className="text-sm font-semibold mb-5 text-fg-default uppercase tracking-wider opacity-60">Organizations</h3>
...
const MetaItem = ({ icon, text }: { icon: React.ReactNode; text: React.ReactNode }) => (
...
const AchievementBadge = ({ icon, bg, title }: { icon: React.ReactNode; bg: string; title: string }) => {
  return (
    <div 
      title={title}
      style={{ background: bg }}
